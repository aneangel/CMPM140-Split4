...
public class Player1Manager : MonoBehaviour
{
    ...
    private void getPlayerInput(){ //Adjusts horizontal movement force depending on player input
        if(Input.GetKey(KeyCode.D)){ //Player will move Right when Right Arrow is pressed
            direction = "right";
            horizontalMovement = 1f;
        } else if(Input.GetKey(KeyCode.A)){ //Player will move Left when Left Arrow is pressed
            direction = "left";
            horizontalMovement = -1f;
        } else{
